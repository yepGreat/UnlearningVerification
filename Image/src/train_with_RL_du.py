...
def train_random_label(model, Du_dataset, Du_loader, Dr_loader, criterion, optimizer,
    ...
    # 获取Du的所有索引
    du_indices = list(range(len(Du_dataset)))
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # 进度条显示每个epoch的训练进度
        progress_bar = tqdm(total=len(du_indices) // Du_loader.batch_size,desc=f'Epoch {epoch}/{num_epochs}')
        ...
        for batch_idx, (inputs, labels) in enumerate(Du_loader):
            ...
            # 为批次中的所有样本生成随机标签
            random_labels = torch.randint(0, num_classes, labels.shape, device=device)

            # 训练步骤
            optimizer.zero_grad()
            outputs = model(inputs)
            loss = criterion(outputs, random_labels)  # 使用随机标签
            loss.backward()
            optimizer.step()
